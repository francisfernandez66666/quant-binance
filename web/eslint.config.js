...
// §LINTGATE（2026-09-24 抄母仓可抄榜项 8）前端 lint 门：一天两例「未定义符号静默蒸发」
// （本仓 Dashboard.jsx 的 setQMTState ReferenceError 被同函数空 catch 吞掉，导致概览页
// 「实盘链路」指示永不渲染且每 15s 静默抛一次；母仓同形态是 Positions.jsx showToast 未导入）
// 是上门的实证理由。**最小集起步**：只开 no-undef 与 no-unused-vars 两条——
...
// English: minimal-start lint gate — only no-undef (error; the silent-ReferenceError class this
// repo actually shipped) and no-unused-vars (warn; too many legacy hits to gate on day one).
import globals from 'globals'

// reactHooksStub：仓内 18 处行内 `eslint-disable(-next-line) react-hooks/exhaustive-deps`
...
// English: a no-op stub so existing inline disable directives resolve without the plugin
// dependency; the rule never reports.
const reactHooksStub = {
  meta: { name: 'react-hooks-stub-for-inline-directives' },
  rules: {
    'exhaustive-deps': {
      meta: { docs: { description: '§LINTGATE 最小集占位：仅服务行内 disable 指令解析，永不上报' } },
      create: () => ({}),
    },
  },
}
...
export default [
  ...
  },
  {
    ...
    languageOptions: {
      globals: {
        ...globals.browser,
        // 测试跑在 node 环境（vitest），有读真实文件的用例（如 cn_master_nav.test.js 用
        // __dirname 定位仓库路径、静态锁类用例读源码文本），故并入 node 全局。
        ...globals.node,
        ...vitestGlobals,
