...
// 日历日变更时补发前一日**——本包没有调度权，不新起定时器；而调用点（§ALERTDRIVE 起是两个：
// internal/engine/scoring_loop.go 的 30s 节流 + cmd/quant/main.go 币安 7×24 维护拍的 60s 一跳）
// 都在会话门禁之前，盘后/休市也照跑，所以日切最多延迟一跳（≤60s），
// 进程停机则重启后首个 tick 补发（按记录的日期标注）。
//
