...
if [[ "$MODE" == "stop" ]]; then
  ...
  # §PICKILL-SCOPE（抄母仓 cf3be1f，2026-09-24 移植）：macOS 的 `pkill -f` 把**环境变量一并计入
  # 匹配串**（同机 `pgrep -fl` 输出里命令行后面拖着整段 env 即证），旧写法
  # `-f "quant.*QUANT_ADDR=:${BACKEND_PORT}"` 会命中**任意一份其它 checkout** 中端口相同的引擎
  # ——本机 quant-binance 与母仓 quant-trading-v2 两份 checkout 端口相同，互相误杀过。
...
# §UAT-PORTS（抄母仓 cf3be1f）：mock 地址必须随端口覆盖一起下发——e2e 里硬编 18789 时，换端口跑会打到
